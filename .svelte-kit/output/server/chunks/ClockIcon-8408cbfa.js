...
const ClockIcon = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<svg xmlns="${"http://www.w3.org/2000/svg"}"${add_attribute("width", size, 0)}${add_attribute("height", size, 0)} fill="${"none"}" viewBox="${"0 0 24 24"}" stroke="${"currentColor"}"${add_attribute("stroke-width", strokeWidth, 0)} stroke-linecap="${"round"}" stroke-linejoin="${"round"}" class="${"feather feather-clock " + escape(customClass, true)}"><circle cx="${"12"}" cy="${"12"}" r="${"10"}"></circle><polyline points="${"12 6 12 12 16 14"}"></polyline></svg>`;
});
